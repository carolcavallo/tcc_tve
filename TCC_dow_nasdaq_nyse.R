...
a <- dfit$mle
dat <- dfit$data
plot((1:length(dat))/length(dat), gevf(a, sort(dat)), xlab = "Empírico",
     ylab = "Modelo", main = "")
abline(0, 1, col = 4)
...
plot(gevq(a, 1 - (1:length(dat)/(length(dat) + 1))), sort(dat),
     ylab = "Empírico", xlab = "Modelo", main = "")
abline(0, 1, col = 4)
...
plot(-1/log(f), q, log = "x", type = "n", xlim = c(0.1, 1000), 
     ylim = c(min(dat, q), max(dat, q)), xlab = "Período de retorno", 
     ylab = "Nível de retorno")
lines(-1/log(f), q)
